...
Use the native grouped tool selector in a Session's bottom panel to switch between Application, Filters, Actions, Logs, and Terminal. Application uses a settings menu in the panel header for Site Data and clearing statistics, cache, or session data. Select **Logs** to follow the Session's activity. Logging runs
